...
public class MongoDAL extends AbstractBaseDAL implements BaseDAL {
	...
	private static final Log LOG = LogFactory.getLog(MongoDAL.class);
	...
	public List<Map<String, Object>> _selectByCriteria(Table table) {
		...
		try {
			DalCriteria dalCriteria = table.getQueryCriteria();
			DB db = database.getDB();
			Jongo jongo = new Jongo(db);
			for(Criteria criteria:dalCriteria.getOredCriteria()){
				for(Criterion criterion:criteria.getAllCriteria()){
					coditon = buildCriteria(criterion, coditon);
				}
			}
			Find find = jongo.getCollection(dalCriteria.getTable()).find(JsonUtils.objToJson(coditon));
		    if(StringUtils.isNotBlank(dalCriteria.getOrderByClause())){
		    	find.sort(dalCriteria.getOrderByClause());
		    }
...
//			else{
//		    	if(dalCriteria.getPageIndex() >= 0){
//		        	int pageSize = 20, pageIndex = 1;
//		        	if(dalCriteria.getPageSize() > 0){
//		        		pageSize = dalCriteria.getPageSize();
//		        	}
//		        	if(dalCriteria.getPageIndex() > 1){
//		        		pageIndex = dalCriteria.getPageIndex();
//		        	}
//
...
//		    }
		    ...
		    if(null != table.getParams()){
				Map<String,Object> map = table.getParams();
		    	for(String fd : map.keySet()){
		        	sb.append(fd).append(",");
		        }
		    }
		    if(sb.length() > 0){
	...
	public int _countByCriteria(Table table) {
		...
		Map<String, Object> coditon = new HashMap<>();
		try {
			DalCriteria queryCriteria = table.getQueryCriteria();
			DB db = database.getDB();
			Jongo jongo = new Jongo(db);
